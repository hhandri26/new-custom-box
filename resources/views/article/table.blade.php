...
<div class="row">
    <div class="pull-right">
        <a href="{{ route('admin_article_form')}}" class="btn btn-sm btn-success">
            <span class="fa fa-plus"></span> Add
        </a>
        <input type="hidden" id="delete" value="{{route('delete') }}?id=">
        <input type="hidden" id="edit" value="{{route('admin_article_edit')}}?id=">
    </div>
          ...
          <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
           <!--  <table class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%"> -->
                ...
                <tbody>
                   ...
                   <tr>
                       <td>{{$no}}</td>
                       <td><img src="{{asset('/public/images/article/'.$row->img)}}" class="img-responsive" alt="" style="width: 200px" ></td>
                       <td>{{$row->name}}</td>  
                       <td>{{$row->category}}</td>  
...
function edit(sid){
  ...
  function deleted(sid){    
    ...
    var url   = url_delete+sid+"&table=t_article&refresh=admin_article";
